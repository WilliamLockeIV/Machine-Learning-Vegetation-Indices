...
Some studies have used machine learning models to optimize or create new vegetation indices, e.g. DeepIndices by Albarracín et al. (2020) and Genetic-Programming-based Vegetation Indices (GPVI) by Vayssade et al. (2021). Although the current research could be used to a similar end, its focus is different. Rather than try to learn an "optimal" vegetation index, it asks the following questions: given the same information as a vegetation index (i.e. the same spectral bands and spatial area) and a remote sensing vegetation task (e.g. plant segmentation), does a machine learning model recreate the relevant vegetation index or learn a new feature representation of its own? If the latter, how can we understand this new representation, along with its causes and consequences for the task at hand? As such, it is more related to AI interpretation than optimization, though by the nature of machine learning its results could also be used for the latter.
...
In this image, the FNN model is obviously correct for almost all pixels where the two thresholds diverge. This is an extreme case, but in general the FNN model will have more correct predictions simply because there is more background than plant life (remember, we trained and evaluated both thresholds on a balanced dataset of plant and background pixels, but in actual photos we will generally have far more background pixels than plants, at least in this dataset).

More interesting is that almost all of the highlighted pixels are in shadow. This gives us a very good indication of what the divergence in the bottom-left corner of Figure 5 corresponds to -- shadows, where both Red and NIR reflectance is low. In these shaded regions, the NDVI threshold over-predicts plant life, while the FNN correctly predicts background for most pixels but still picks out the handful of plants.

![20200720_121239_corn_0100](Images/20200720_121239_corn_0100.png)
**Figure 7: Corn with disturbed soil**
...
## Discussion
Visualizing how the different thresholds partition their input space (Fig 5) allows us to see that the NDVI threshold and FNN threshold are actually very similar over most possible pixel values, except in the bottom-left corner where both Red and NIR values are low. Looking at pixels that fall within that range in real images (Figs 6-11) shows us that this corresponds to shadows in the image, which are a common confounding factor in calculating and using vegetation indices (Vaysadde et al., 2021; Zhang et al., 2015). Within these shaded regions, the FNN requires a higher ratio of NIR to Red bands (effectively a higher NDVI threshold) to classify a pixel as belonging to plant life. This is actually supported by Zhang et al. (2015), who find that plants under shade have a higher NDVI value than in direct sunlight, so it is appropriate to set a higher NDVI threshold separating vegetation from background in that setting. Note that this is something a single NDVI threshold, by its very nature, cannot do.

Interestingly, this does not correspond to a higher F1 score on the Test dataset. This might be in part due to the random split of Train and Test images -- in the particular split used for these results, the Test dataset had a higher percentage of vegetation pixels fall within this low-Red, low-NIR region of the input space than the Train dataset, so the model may have over-estimated the threshold it should draw for these shaded regions. Out of ten other random initializations, in the five where the Train and Test datasets had roughly the same proportion of vegetation to background pixels in these shaded regions, the FNN performed better than the NDVI threshold, though still only by 1% F1 score.

Nonetheless, even as the machine learning model does not bring a significant improvement in performance over the baseline, it is edifying to know that it does learn a feature representation equivalent to NDVI thresholding in direct sunlight, and it adjusts this representation in shade in a manner supported by our empirical understanding of the effects of shadows on NDVI.

# Future Research
It would be straightforward to apply this model and technique to other two-band vegetation indices, e.g. the Simple Ratio (SR), the Two-Band Enhanced Vegetation Index (EVI2), or the Green Normalized Difference Vegetation Index (GNDVI). However, given that many of these use the same Red and NIR channels as NDVI, we might not get much new information from this. 

More significantly, I'd like to extend these techniques to more commonly used machine learning models and a broader array of remote sensing tasks. Most machine learning models take more than two inputs, and most remote sensing tasks look at more than a single pixel at a time. The mathematical basis for this research, the spline theory of neural networks (Balestriero & Baraniuk, 2018a,b), can be applied to a wide range of neural networks with an arbitrary number of inputs and outputs, so from a theoretical perspective it is very generalizable. However, the analysis and interpretation of results in the pilot study is based heavily on visualization -- how the two thresholds split up the input space (Fig 5) and the pixels in actual images that correspond to different predictions between models (Figs 6-11). The first of these (which informs the second) is only possible because we have a two-dimensional input space, a Red value and an NIR value for a single pixel. As we start to add more spectral bands and more pixels, the input dimension expands, and we would have to find new intepretive tools to analyze our mathematical results. Some of these tools already exist, e.g. SplineCam (Humayun et al., 2023), which visualizes partitions on 2D slices of the input space (similar to Fig 5), and local complexity (Humayun et al., 2024), which measures the concentration of partitions near data points and serves as a progress measure of model generalization and robustness. We would still have to evaluate these tools in the domain of vegetation indices and remote sensing, and possibly develop some of our own. However, I'm optimistic that this framework of comparing machine learning representations and vegetation indices can give us new insights into how both humans and neural networks accomplish remote sensing tasks.
...
Zhang, L., Sun, X., Wu, T., & Zhang, H. An Analysis of Shadow Effects on Spectral Vegetation Indexes Using a Ground-Based Imaging Spectrometer. IEEE Geoscience and Remote Sensing Letters, 2015.
